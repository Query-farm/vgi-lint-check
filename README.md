...
| Columns | VGI2xx | column-comment coverage + **every column commented**, comment-not-echo, **naive TIMESTAMP documents its timezone**, **a column name maps to one SQL type across tables** |
| Functions | VGI3xx | description (+ quality), documented parameters, named arguments, **per-argument descriptions required** (error) that **don't restate the data type** (warning; needs a vgi extension exposing `vgi_function_arguments()`); **function docs must not re-document their arguments** (a parameter list in the function doc duplicates `vgi_function_arguments()` and drifts); examples, scalar-function stability (all-VOLATILE smell + per-function VOLATILE flag), **every-parameter-ANY smell**, **parameterless table function should be a table**, **an argument name maps to one SQL type across functions**; **a lone multi-dimensional-array argument should be a table input**; **an argument whose docs enumerate allowed values or a numeric range should declare machine-readable constraints (`choices`/`ge`/`le`) for agent discovery**; **a declared default must satisfy its own constraints** (error), **declared constraints must be well-formed** (valid regex, non-empty range), and **a `choices` set must offer more than one value** |
...
| Structure | VGI11x/13x/14x | **schema not empty**; warn on excessive table/function counts and over-long table/function names; **schema object-count cap (>50 by default)**; **redundant `get_`/`list_` name prefixes**; **consistent name style (snake_case) + consistent table singular/plural** |
