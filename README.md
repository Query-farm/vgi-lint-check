...
| Functions | VGI3xx | description (+ quality), documented parameters, named arguments, **per-argument descriptions** (needs a vgi extension exposing `vgi_function_arguments()`), examples, scalar-function stability (all-VOLATILE smell + per-function VOLATILE flag), **every-parameter-ANY smell**, **parameterless table function should be a table** |
