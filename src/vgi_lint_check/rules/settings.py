    ...
    def check(self, ctx):
        for s in ctx.catalog.settings:
            if blank(s.description):
                yield self.finding(
                    ctx, s.id, f"setting {s.name!r} has no description",
    ...
    def check(self, ctx):
        ...
        for s in ctx.catalog.settings:
            ...
            if is_trivial_echo(d, s.name):
                yield self.finding(
                    ctx, s.id, f"setting description just restates {s.name!r}",
                    "describe what the setting does, not just its name",
                )
            elif len(d.strip()) < minlen:
                yield self.finding(
                    ctx, s.id,
                    f"setting description is very short ({len(d.strip())} < {minlen})",
