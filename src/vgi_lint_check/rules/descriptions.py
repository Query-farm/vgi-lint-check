...
class SchemaComment(Rule):
    ...
    def check(self, ctx):
        for s in ctx.catalog.iter_schemas():
            if blank(s.comment):
                yield self.finding(
                    ctx, s.id, "schema has no comment",
...
class LLMDescription(Rule):
    ...
    summary = (
        "Tables/views should carry a 'vgi.description_llm' tag for LLM consumers."
    )
    ...
    def check(self, ctx):
        for t in ctx.catalog.iter_table_like():
            if blank(t.description_md):
                yield self.finding(
                    ctx, t.id, f"missing '{TAG_DESCRIPTION_MD}' tag",
...
class MarkdownNotIdenticalToLLM(Rule):
    ...
    def check(self, ctx):
        for t in ctx.catalog.iter_table_like():
            ...
            if not blank(llm) and not blank(md) and llm.strip() == md.strip():
                yield self.finding(
                    ctx, t.id,
